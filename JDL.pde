//import processing.serial.*; //This is a library we can use for serial communication with the uCPU - Supports RS-232


//---------------------------------------------------------------------------


Star[] stars; //initialize array of Star objects (used for start menu)
ChessPiece[][] board; // Initialize 2d array of ChessPiece Objects
Engine stockfish;
String path = "C:\\Users\\colin\\Desktop\\Chess engine\\stockfish_20090216_x64_bmi2.exe"; //Path for UCI Chess engine

//Initialize button objects
Button start_button; 

int boardSize = 800;
float gridSize = boardSize/8;
int pieceSize = (int)gridSize;

int game_state = 2; //initialize game state variable used to toggle between (game, menu, endgame, etc)

//A string storing the current board state in FEN notation
String cur_fen = "2k3r1/pbpp2Rp/1p6/1P3P2/1b6/2N5/PBPP1PPP/4R1K1 w - - 0 19";
String temp_fen = "\0"; // temporary variable for pulling a new fen from the engine
Boolean whites_turn = true;

void setup() {
  size(1200,800);
  background(50,50,70);
  
  int dia = int(random(1,5));
  int numbStars = width;
  stars = new Star[numbStars];
  for(int i = 0; i < stars.length; i++){
    float x = random(width);
    dia = int(random(1,5));
    stars[i] = new Star(x,random(height),dia,dia, random(10,255));
  }
  start_button = new Button("Start Game", width/2, height/2, 280, 75, color(255), color(0));
  stockfish = new Engine(path);
  stockfish.init();
  stockfish.listenTwo();
  delay(111);
  stockfish.say("isready");
  delay(2000);
  stockfish.listenTwo();
  noStroke();
  
  board = new ChessPiece[8][8];
  readFen(cur_fen);
  //drawPieces();
}

void draw() {  
  switch(game_state){
  //Start Menu
  case 0:
    startMenu();
    start_button.display();
  break;
  //Setup Menu
  case 1:
    
  break;
  case 2:
    drawBoard("white");
    drawPieces();
    exampleCPUAnal();
  break;
  default:
  }
  
  
}//end "draw" function

//Function to draw the chess board itself, string for which side the player is on
void drawBoard(String player_color){
  color g_color;
  color g_color_w = color(246, 232, 177);
  color g_color_b = color(10, 130, 30);
  int num = 0;
  if(player_color == "white") {
    g_color = g_color_w;
  }else{
    g_color = g_color_b;
  }
  
  for(float g_y = 0; g_y<boardSize; g_y+=gridSize){
      for(float g_x = 0; g_x<boardSize; g_x+=gridSize){
        fill(g_color);
        noStroke();
        rect(g_x,g_y,gridSize,gridSize);
        fill(0);
        //text(num, g_x+20,g_y+20);
        //Change the color after each square
        if(g_color == g_color_b){
          g_color = g_color_w;
        }else{
          g_color = g_color_b;
        }

        num++;
        
      }//inner for loop
      
      //Change the color after each square
        if(g_color == g_color_b){
            g_color = g_color_w;
          }else{
            g_color = g_color_b;
          }
  }//outer for loop
}// end "drawBoard"


//Function to read a fen string and initialize required variables for legal move logic
void readFen(String curFen){
  String fen = curFen;
  int row = 0;
  int col = 0;
  
  //First for loop exclusively for reading piece postion, forecully terminates upon reading a ' ' (space)
  for(int i = 0; i<fen.length(); i++){
    //println( (int) fen.charAt(i));
    if(fen.charAt(i) == '/'){
      row++;
      col=0;
    }else if( (int)fen.charAt(i) <= 114 && (int)fen.charAt(i) >= 65) {
      board[col][row] = new ChessPiece(fen.charAt(i), col*gridSize+(gridSize/2), row*gridSize+(gridSize/2), pieceSize, 8*row+col);
      col++;
    }else if( (int)fen.charAt(i) <= 56 && (int)fen.charAt(i) >= 49) {
          col += ((int)fen.charAt(i)-48);
    }else if(fen.charAt(i) == 'w') {
      whites_turn = true;
    }else if(fen.charAt(i) == 'b') {
      whites_turn = false;
    }
    
  }
}

void drawPieces() {
  for (int i = 0; i<8; i++){
    for (int j = 0; j<8; j++) { 
      if (board[i][j] != null){
        board[i][j].display();//piece
        board[i][j].MouseIsOver();
        board[i][j].move();
      }
    }
  }
}

/*
  Function for displaying the start menu
  This is what the user will be greated with when they first launch the program
*/
void startMenu() {
  background(0);
  //fill(255);
  for(int i = 0; i < stars.length;i++){
    stars[i].move();
    stars[i].display();
  }
  
  textAlign(CENTER, CENTER);
  textSize(100);
  fill(100,100,200);
  text("Robot Chess", width/2, height/4);
  
}

//Built in processing function that runs whenever the mouse is clicked.
void mousePressed() {
  for (int i = 0; i<8; i++){
    for (int j = 0; j<8; j++) { 
      if (board[i][j] != null){
        if(board[i][j].MouseIsOver()) {
          board[i][j].selected = true;
        }
        
      }
    }
  }
  // If the start menu is pressed, advance the game menu.
 if(start_button.MouseIsOver() && game_state !=2) {
  background(255);
  game_state = 2; 
 }
 
} //end of mousePressed

void mouseReleased() {
    for (int i = 0; i<8; i++){
    for (int j = 0; j<8; j++) { 
      if (board[i][j] != null){
        if(board[i][j].MouseIsOver() && mouseX < boardSize && mouseY < boardSize) {
          board[i][j].selected = false;
          board[i][j].x = int(mouseX/gridSize)*(gridSize)+gridSize/2;
          board[i][j].y = int(mouseY/gridSize)*(gridSize)+gridSize/2;
        }
        
      }
    }
  }
}


void exampleCPUAnal(){
  float cpuAnal = 15;
  float cpuY = 60;
  float cpuX = 870;
  float playerY = 180;
  float playerX = 870;
  
  //Black Clock and player
  textSize(25);
  fill(220);
  rect(boardSize, 0, (width-boardSize), (height)); 
  fill(0);
  text("Black: CPU (3200)", cpuX, cpuY-5);
  fill(50);
  rect(cpuX, cpuY, 200, 50, 10);
  fill(255);
  textSize(40);
  text("06:12", cpuX+50, cpuY+40);
  
  
  //White Clock and Player
  textSize(25);
  fill(0);
  text("White: Player (900)", playerX, playerY-5);
  fill(50);
  rect(playerX, playerY, 200, 50, 10);
  fill(255);
  textSize(40);
  text("05:37", playerX+50, playerY+40);
  
  
  
  //Indicator Bar
  fill(0);
  rect(boardSize, 0, 50, cpuAnal);
  fill(255);
  rect(boardSize, cpuAnal, 50, height);
  textSize(20);
  fill(0);
  text("+M2", boardSize, height/2);
  
  
  text("Indication for best moves here", boardSize+ 50, 300);
  
  text("Menu Buttons down here", boardSize+50, height-70);
  
}
//Star class for start menu background
class Star {
  float x, y;
  float diameter;
  float speed;
  float direction = 1;
  float fade;
  Star(float xpos, float ypos,float dia, float sp,float f){
    x = xpos;
    y = ypos;
    diameter = dia;
    speed = sp*0.3;
    fade = f; 
  }
  
  void move() {
    x += speed * direction;
    if(x >= width){
      x = 0;
      y = random(height);
    }
  }
  
  void display() {
    fill(255, fade);
    ellipse(x,y,diameter,diameter);
  }
}

//Button class for creating menus
class Button {
 float x, y, w, h;
 color c, t;
 String text;
 Button(String txt, float xp, float yp, float wt, float lt, color back_color, color txt_color) {
   text = txt;
   x = xp;
   y = yp;
   w = wt;
   h = lt;
   c = back_color;
   t = txt_color;
 }
 void display() {
   fill(c);
   rect(x-(w/2),y-(h/2),w,h,10);
   fill(t);
   textAlign(CENTER, CENTER);
   textSize(50);
   text(text, x,y);
 }
 
 //Function for determining if the mouse is over the button.
 boolean MouseIsOver() {
    if (mouseX > x-(w/2) && mouseX < x+(w/2) && mouseY > y-(h/2) && mouseY < y+(h/2)) {
      return true;
    }
    return false;
  }
 
}
